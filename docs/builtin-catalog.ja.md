...
| | `takt-default-refresh-fast` | TAKT 開発 workflow の refresh 最適化版。`reasoning_effort` や loop 条件は変えず、`write_tests`・`ai_review`・各 reviewer・`fix` のような文脈肥大しやすい step にだけ `session: refresh` を追加する。 |
